...
/**
 * M5: rate-limit/transient retry with exponential backoff.
 *
...
describe("retry", function () {
  ...
  describe("withRetryProvider", function () {
    it("retries a flaky provider transparently and preserves id", async function () {
      let attempts = 0;
      const flaky: RelevanceProvider = {
        id: "flaky",
        async scoreItems(items) {
          attempts++;
          if (attempts < 2) throw new ProviderError("429", 429);
          return items.map((it) => ({
            itemKey: it.itemKey,
            score: 1,
            reason: "",
      ...
      const wrapped = withRetryProvider(flaky, {
        sleep: async () => {},
        rng: () => 0,
      });
      assert.equal(wrapped.id, "flaky");
      const out = await wrapped.scoreItems(
        [{ itemKey: "A", title: "", abstract: "" }],
        "p",
      );
