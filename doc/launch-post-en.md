...
_One sortable numeric column to answer the question Zotero never could: "what do I read first?"_
...
- **Reading-status plugins** (zotero-reading-list, Reading Flow) — solve _status_
...
- A **context menu** (right-click → _Zotero Triage_): quick levels (High / Medium / Low),
...
3. **Numeric sorting isn't free.** The item tree sorts the column by the _string_ returned.
...
| Automated tests      | **14/14 green**, running _inside_ a real Zotero 9              |
...
> **Not measured yet (full honesty):** the _relevance benchmark_ for the auto-ranking layer
...
All behind a _provider_ interface, so embeddings/LLMs (opt-in) become just a pluggable
alternative later.
...
_No telemetry. No network. Your device, your data._
